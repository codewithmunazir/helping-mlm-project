      ...
      <div class="container-fluid">
        ...
        <div class="row">
         ...
         <div class="col-lg-12 col-12">
            <label for="provider_id">Filter by Provider ID:</label>
<input type="text" id="provider_id" placeholder="Enter Provider ID">

<label for="receiver_id">Filter by Receiver ID:</label>
<input type="text" id="receiver_id" placeholder="Enter Receiver ID">
...
<script src="<?php echo base_url();?>Assets_s/dist/js/pages/dashboard2.js"></script>
    ...
    function load_data(provider_id = '', receiver_id = '', page = 1) {
        $.ajax({
            url: base_Url + 'Admin/fetch_data_coomit_all_get_provide',
            type: "POST",
            data: {
            ...
            success: function(data) {
                ...
                if (data.records.length > 0) {
                    $.each(data.records, function(index, record) {
                        ...
                        var slip;
                        var stts;
                        if (record.slip_uplode != null && record.request_status == 1) {
                            stts = "Approved";
                        } else if (record.slip_uplode != null && record.request_status == 2) {
                            stts = "Slip uploaded";
                        } else if (record.slip_uplode == null && record.request_status == 2) {
                            stts = "Pending";
                        } else {
                        ...
                        if (record.slip_uplode != null) {
                            slip = '<a href="' + base_Url + '../uploads_users/' + record.slip_uplode + '" target="_blank" class="btn btn-info">View Payment Slip</a>';
                        } else {
                            slip = 'Not uploaded';
                        }

                        if (record.request_status == 1) {
                            appr = record.g_registeruser_id + '<br>' + record.gfullname;
                        } else if (record.request_status == 2) {
                            appr = "Pending";
                        } else if (record.request_status == 0) {
                            appr = "Rejected";
                        } else {
                            appr = '  ';
                        }
                        ...
                        var row = '<tr>' +
                            '<td>' + record.id + '</td>' +
                            '<td>' + record.p_registeruser_id + '<br>' + record.pfullname + '</td>' +
                            '<td>' + record.g_registeruser_id + '<br>' + record.gfullname + '</td>' +
    ...
    $('#filter').on('click', function() {
        ...
        load_data(provider_id, receiver_id, current_page);
    });
